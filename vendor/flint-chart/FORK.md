...
Everything below is additive; no upstream logic was rewritten.

| File | Change |
|---|---|
| `src/highcharts/**` | **New backend**: `assemble.ts`, `instantiate-spec.ts`, `colormap.ts`, `templates/{index,utils,bar,line,area,scatter,pie}.ts`, `README.md` |
...
| `tests/highcharts.test.ts` | **New**: smoke tests for the five chart types |
...
Supported: Bar, Line, Area, Scatter, Pie — exactly ChartBrain's neutral-spec
whitelist. Not supported: faceting, chart-type transforms, native datetime
axes, size channel on scatter. See `src/highcharts/README.md` for details.
